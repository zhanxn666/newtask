...
class MFFN(BasicModelClass):
    ...
    def encoder_translayer(self, x):
        ...
        return trans_feats
    def body(self, c1_scale, o_scale, c2_scale, a1_scale, a2_scale):#real forward method
        ...
        # Hierarchical Channel Fusion Decoder
        x = self.d5(feats[0])#CFU
        ...
        x = self.d1(x + feats[4])
        x = cus_sample(x, mode="scale", factors=2)
        # 保存最终 x 的输出(modified by me)
        final_features = x.clone()  # [B, 64, 384, 384]
        #convblock
        logits = self.out_layer_01(self.out_layer_00(x))
          ## logits最终分割结果 [B, 1, 384, 384] final_features最终特征图 [B, 64, 384, 384](modified by me)
        return dict(seg=logits, final_features=final_features)
