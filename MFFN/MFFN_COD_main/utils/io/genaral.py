...
def get_datasets_info_with_keys(dataset_infos: list, extra_keys: list) -> dict:
    ...
    def _get_info(dataset_info: dict, extra_keys: list, path_collection: defaultdict):
        ...
        # === MODIFIED: Skip mask intersection if mask dir doesn't exist or is dummy ===
        if "mask" in total_keys:
            mask_dir = infos["mask"]["dir"]
            if os.path.exists(mask_dir) and os.listdir(mask_dir):  # real mask dir with files
                mask_names = get_name_list_from_dir(mask_dir)
                image_names = _get_intersection(image_names, mask_names)
            else:
                # Fake/dummy mask dir → keep ALL image names
                print(f"  [YOLO MODE] Skipping mask filtering (dummy: {mask_dir})")
